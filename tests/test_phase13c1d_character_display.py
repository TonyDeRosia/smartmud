...
def _char(**kw):
    base=dict(name='Kraevok', title='Adventurer', race='Human', character_class='Adventurer', level=1, hp=46, max_hp=100, mana=50, max_mana=50, stamina=100, max_stamina=100, xp=0, gold=0, posture='standing')
    base.update(kw)
...
def test_score_and_worth_use_classic_frame_without_placeholders():
    ...
    assert 'Base Stats:' in score and 'Gold: 0' in score and 'Posture: standing' in score
...
def test_skills_and_spells_are_distinct_structured_displays():
    ...
    spell={'id':'recall','name':'Recall','ability_type':'spell','rank':1,'maximum_rank':100,'status_text':'Ready','costs':[{'resource_id':'mana','amount':5}], 'targeting': {'mode':'self'}, 'description':'Return safely to your configured recall point.'}
