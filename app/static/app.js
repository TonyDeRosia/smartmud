...
function mudUpdateChrome(title, subtitle) {
...
function mudRenderWorldSelect() { mudUpdateChrome('World Select', 'Smart MUD · World Select'); if (statusWorldLocation) statusWorldLocation.textContent = 'Choose a world'; if (dialogueFeed) { dialogueFeed.innerHTML = `<section class="mud-world-select"><h2>World Select</h2><div class="mud-card-grid">${mudWorlds.map(mudWorldCard).join('')}</div></section>`; dialogueFeed.querySelectorAll('button[data-mud-world]').forEach((b) => { b.onclick = () => mudSelectWorld(b.dataset.mudWorld); }); } if (selectedSaveLabel) selectedSaveLabel.textContent = 'Worlds'; }
...
function mudRenderCharacterSelect() { mudUpdateChrome(mudSelectedWorld.name, `${mudSelectedWorld.name} · Character Select`); if (statusWorldLocation) statusWorldLocation.textContent = 'Character Select'; const cards = mudCharacters.map((ch) => `<article class="mud-card"><h3>${escapeHtml(ch.name)}</h3><p>${escapeHtml(ch.race)} ${escapeHtml(ch.class)} · Level ${escapeHtml(String(ch.level || 1))}</p><p>Current room: ${escapeHtml(ch.current_room || '')} · Last played: ${escapeHtml(ch.last_played || '')}</p><button type="button" data-enter-character="${escapeHtml(ch.character_id)}">Enter World</button><button type="button" data-delete-character="${escapeHtml(ch.character_id)}">Delete Character</button></article>`).join('') || '<p>No characters exist for this world yet.</p>'; dialogueFeed.innerHTML = `<section class="mud-character-select"><h2>Existing Characters</h2><div class="mud-card-grid">${cards}</div><button id="mud-create-character" class="btn-primary" type="button">Create Character</button></section>`; document.getElementById('mud-create-character').onclick = mudRenderCharacterCreator; dialogueFeed.querySelectorAll('button[data-enter-character]').forEach((b) => { b.onclick = () => mudEnterCharacter(b.dataset.enterCharacter); }); dialogueFeed.querySelectorAll('button[data-delete-character]').forEach((b) => { b.onclick = () => mudDeleteCharacter(b.dataset.deleteCharacter); }); if (!mudCharacters.length) mudRenderCharacterCreator(); }
...
function mudRenderCharacterCreator() { const races = mudSelectedWorld.races || [], classes = mudSelectedWorld.classes || []; dialogueFeed.innerHTML = `<section class="mud-character-create"><h2>Create Character</h2><label>Character name <input id="mud-character-name" type="text" /></label><h3>Race</h3><div class="mud-card-grid">${mudOptionCards(races, 'mud-race')}</div><h3>Class</h3><div class="mud-card-grid">${mudOptionCards(classes, 'mud-class')}</div><label>Appearance description <textarea id="mud-appearance" rows="4"></textarea></label><aside class="mud-readonly-details"><h3>Read-only starting data</h3><p>Stats, abilities, and equipment come from world data and cannot be edited.</p></aside><button id="mud-create-submit" class="btn-primary" type="button">Create Character</button></section>`; document.getElementById('mud-create-submit').onclick = mudCreateCharacter; }
...
async function mudRefreshPlayView() { const data = await api('/api/mud/play-view'); mudUpdateChrome(data.world?.name || 'World', `${data.world?.name || 'World'} · ${data.character?.name || 'Character'} · ${data.room?.name || 'Room'}`); if (statusWorldLocation) statusWorldLocation.textContent = data.room?.name || 'Room'; if (statusTurnCount) statusTurnCount.textContent = String(data.character?.level || 1); dialogueFeed.innerHTML = `<pre class="mud-terminal-output">${escapeHtml(data.output || '')}</pre>`; }
async function mudSendInput() { const input = document.getElementById('chat-input'); const text = input.value.trim(); if (!text) return; input.value = ''; await api('/api/mud/input', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ text }) }); await mudRefreshPlayView(); }
