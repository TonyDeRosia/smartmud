...
class WorldRegistry:
    def __init__(self, worlds_dir: Path | None = None) -> None:
        ...
        self._package_cache: dict[str, tuple[str, WorldPackage]] = {}

    def _world_content_hash(self, root: Path) -> str:
        import hashlib
        h = hashlib.sha256()
        for path in sorted([p for p in root.rglob("*") if p.is_file() and "builder" not in p.parts]):
            if path.suffix.lower() not in {".json", ".md"}:
                continue
            h.update(str(path.relative_to(root)).encode("utf-8")); h.update(b"\0")
    ...
    def load_world(self, world_id: str) -> WorldPackage:
        root = self.worlds_dir / world_id
        content_hash = self._world_content_hash(root)
        cached = self._package_cache.get(world_id)
